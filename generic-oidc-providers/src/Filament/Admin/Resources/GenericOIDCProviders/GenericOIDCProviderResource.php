        ...
        return $schema
            ...
            ->components([
                ...
                Toggle::make('verify_jwt')
                    ...
                    ->stateCast(new BooleanStateCast(false))
                    ->live(),
                Toggle::make('use_jwks_discovery')
                    ->label(trans('generic-oidc-providers::strings.use_jwks_discovery'))
                    ->helperText(trans('generic-oidc-providers::strings.use_jwks_discovery_help'))
                    ->inline(false)
                    ->onIcon('tabler-check')
                    ->offIcon('tabler-x')
                    ->onColor('success')
                    ->offColor('danger')
                    ->stateCast(new BooleanStateCast(true))
                    ->visible(fn (Get $get) => $get('verify_jwt'))
                    ->live(),
                Textarea::make('jwt_public_key')
                    ->label(trans('generic-oidc-providers::strings.jwt_public_key'))
                    ->visible(fn (Get $get) => $get('verify_jwt') && !$get('use_jwks_discovery'))
                    ->columnSpan(2)
                    ->rows(3)
                    ->autosize()
